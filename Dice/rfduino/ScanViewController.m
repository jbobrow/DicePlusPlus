...
- (void)viewDidLoad
{
    ...
    int numberOfLines = 3;
    self.tableView.rowHeight = 200.f;//(44.0 + (numberOfLines - 1) * 19.0);
...
{
    // TODO: Make my own cell with an image of dice
    // data listed (signal strength... ok
    // activity shown as animation (blinking circles)
    // status on color of dice
    // green dice = good signal
    // red dice = bad signal
    ...
    if (! cell) {
        ...
        GLModelView *model = [[GLModelView alloc] initWithFrame:frame];
        model.texture = nil;
        model.backgroundColor = [UIColor blackColor];
        model.blendColor = [UIColor whiteColor];   //colorWithRed:0.f green:.5f blue:1.f alpha:1.f];
        model.model = [GLModel modelWithContentsOfFile:@"dice_round.obj"];
        
        //set default transform
        CATransform3D transform = CATransform3DMakeTranslation(0.f, 0.f, -5.0f);
        transform = CATransform3DRotate(transform, M_PI_4, 1.f, 0.f, 0.f); // rotate around x
        model.modelTransform = transform;
        transform = CATransform3DRotate(transform, M_PI_4, 0.f, 1.f, 0.f); // rotate around y
        model.modelTransform = transform;
